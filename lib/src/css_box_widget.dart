...
class CSSBoxWidget extends StatelessWidget {
  ...
  bool _shouldExpandToFillBlock() {
    return (style.display == Display.BLOCK || style.display == Display.LIST_ITEM) && !childIsReplaced && !shrinkWrap;
  }
...
class _RenderCSSBox extends RenderBox
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! CSSBoxParentData) child.parentData = CSSBoxParentData();
  }
  ...
  _Sizes _computeSize({required BoxConstraints constraints, required ChildLayouter layoutChild}) {
    ...
    final childConstraints = constraints.copyWith(
      maxWidth: (this.width.unit != Unit.auto)
          ? this.width.value
          : containingBlockSize.width - (this.margins.left?.value ?? 0) - (this.margins.right?.value ?? 0),
      maxHeight: (this.height.unit != Unit.auto)
          ? this.height.value
          : containingBlockSize.height - (this.margins.top?.value ?? 0) - (this.margins.bottom?.value ?? 0),
...
double _calculateEmValue(Style style, BuildContext buildContext) {
  ...
  return (style.fontSize?.emValue ?? 16) *
      MediaQuery.textScalerOf(buildContext).scale(1) *
